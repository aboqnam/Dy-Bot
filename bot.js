...
client.on('ready', function(){
    ...
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`https://www.twitch.tv/boqnam`);
    }, ms);
 ...
 client.on('message', message => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .setColor('RANDOM')
            .setTitle('``INFO BoQnam Community ©`` ')
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'BoQnam Community ©\'s translator',
...
if (message.content.startsWith(prefix + 'tag')) {
    let args = message.content.split(" ").slice(1);
if(!args[0]) return message.reply('مرجو كتابة النص الذي تريده');  
...
client.on('message', message => { 
    ...
    message.reply('** أنتهيت ? **').then(msg => {msg.delete(10000)});
    }
    });
    client.on('message', message => {
    let args = message.content.split(' ').slice(1);
...
let args = message.content.split(" ").join(" ").slice(2 + prefix.length);
let request = `Requested By ${message.author.username}`;
message.channel.send(`**هل انت متاكد انك تريد مسح الرسائل?**`).then(msg => {
msg.react('✅')
.then(() => msg.react('❌'))
...
let reaction2 = msg.createReactionCollector(reaction2Filter, { time: 12000 });
reaction1.on("collect", r => {
message.channel.send(`✅تم مسح الرسائل`).then(m => m.delete(5000));
var msg;
        msg = parseInt();
...
reaction2.on("collect", r => {
message.channel.send(`**✅تم الغاء العملية**`).then(m => m.delete(5000));
msg.delete();
})
})
}
...
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("**لاتملك  `MANAGE_ROLES` صلاحية**").then(msg => msg.delete(6000))
...
client.on('message', message => {
  ...
  if(!message.guild.member(message.author).hasPermission("KICK_MEMBERS")) return message.reply("**انت لاتملك ` KICK_MEMBERS ` صلاحية**");
  if(!message.guild.member(client.user).hasPermission("KICK_MEMBERS")) return message.reply("**انت لاتملك ` KICK_MEMBERS ` صلاحية**");
  let user = message.mentions.users.first();
